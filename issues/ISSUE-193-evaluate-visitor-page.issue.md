...
"Land approved visitor pages (evaluate, model, adopt, entity, pack, reference lifecycle)".
The issue file itself was authored on the visitor-docs branch (`a3443cd`) and was not
included in that merge — restored here so the reserved tag `backstop/issue/193` has a
corpus file. Closes via `resolved-by` on the merge commit. `PLAN-ISSUE-193` remains
`status: draft` on disk; flipping it to `completed` is a separate plan-closeout pass
