...
func copyPagesContract(t *testing.T) string {
	t.Helper()
	root := t.TempDir()
	for _, relative := range []string{".github/pages-actions.lock.yml", ".github/workflows/pages.yml", ".github/workflows/site-verification.yml"} {
...
func TestSiteCheck_SiteVerificationHasNoDeploymentAuthority(t *testing.T) {
	workflow, err := os.ReadFile(filepath.Join(repositoryRoot(), ".github/workflows/site-verification.yml"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestSiteCheck_RejectsParallelTruthOrPublication(t *testing.T) {
	for _, relative := range []string{".github/workflows/pages.yml", ".github/workflows/site-verification.yml"} {
		workflow, err := os.ReadFile(filepath.Join(repositoryRoot(), relative))
		if err != nil {
			t.Fatal(err)
		}
		text := string(workflow)
		for _, forbidden := range []string{"peaceiris/actions-gh-pages", "jekyll-action", "docs/index.html --destination", "git push gh-pages"} {
			if strings.Contains(text, forbidden) {
				t.Fatalf("%s contains parallel publication surface %q", relative, forbidden)
			}
		}
	}
}
