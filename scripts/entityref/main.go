...
func main() {
	...
	overlayPath := filepath.Join(root, "docs/_data/entity-reference.yml")
	...
	for _, ent := range overlay.Entities {
		...
		if err != nil {
			fatal(fmt.Errorf("%s: %w", ent.ID, err))
		}
		out := filepath.Join(root, ent.Output)
		if check {
			old, err := os.ReadFile(out)
			if err != nil {
				fatal(fmt.Errorf("%s: %w", ent.Output, err))
			}
			if string(old) != page {
				fatal(fmt.Errorf("%s drifted from schema+overlay; run go run ./scripts/entityref", ent.Output))
			}
			fmt.Printf("entityref: ok %s\n", ent.Output)
			continue
		}
		if err := os.WriteFile(out, []byte(page), 0o644); err != nil {
			fatal(err)
		}
		fmt.Printf("entityref: wrote %s from %s\n", ent.Output, schemaSource(ent))
	}
}
...
func repoRoot() string {
	...
	fatal(fmt.Errorf("go.mod not found from %s", wd))
	return ""
}
...
func render(root string, ent Entity) (string, error) {
	...
	if ent.StatusPath != "" || len(ent.StatusMeanings) > 0 {
		...
		if len(values) == 0 {
			for _, k := range sortedKeys(ent.StatusMeanings) {
