...
'docs/model.md':{
 ...
 'The proactive track decomposes a bundle into independently accepted specifications. The reactive track turns a bounded issue directly into a plan. The distinction keeps discovery work from masquerading as an implementation task.':'canonical concept definition',
 'A plan names task order, test names, file scope, gates, and dependencies. Implementers execute that contract; they do not silently redesign it.':'canonical concept definition',
 'Packs are versioned standards declarations. They own rules, engines, fixtures, and findings while Core remains the thin executor that resolves and runs them.':'canonical concept definition',
 'Recipes compose repeatable repository operations without transferring standards ownership into Core.':'canonical concept definition',
 'A gate resolves installed packs, runs their declared engines, applies severity and policy, and emits a blocking or passing verdict.':'canonical concept definition',
 'A baseline records accepted existing debt. A ratchet prevents new debt from expanding it, so adoption can start honestly without making regression permissible.':'canonical concept definition',
 'Waivers are explicit, attributable, and time-bounded exceptions. They preserve the governing standard instead of weakening it invisibly.':'canonical concept definition',
 'Capabilities state outcomes. User journeys demonstrate those outcomes from a visitor or operator perspective against built behavior.':'canonical concept definition',
 'Evidence binds a claim or verdict to immutable source, exact execution, and governing intent. Green without provenance is not a durable delivery fact.':'canonical concept definition',
 ...
 'Core owns execution and lifecycle primitives. Packs own standards and engines. Harnesses own orchestration. External toolchains own their behavior. The authoritative boundary view is `docs/_diagrams/ARCH-003-ownership-boundaries.mmd`.':'canonical ownership definition'},
'docs/adopt.md':{
