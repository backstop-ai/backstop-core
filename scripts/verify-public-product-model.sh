...
'docs/model.md':{
 'Name the work before the agent writes code. New work goes through a bundle, a spec, and a plan. A bounded fix can skip to an issue and a plan. Both tracks stop at a reviewed plan. Nothing is implemented before that.':'visitor operating guide',
 ...
 'Turn the decisions you would fail a merge over into rules the agent cannot skip. Encode them once. The agent runs them while it works. CI runs the same gate later.':'visitor operating guide',
 '<div class="guide-list">\n<article>\n<h3>Packs</h3>\n<p>Versioned standards with engines and fixtures. Core stays a thin executor: it only runs what a pack declares.</p>\n</article>\n</div>':'visitor operating guide',
 'Pinned, repeatable operations — a project, a pipeline, a config — that do not move the standard into Core.':'visitor operating guide',
 'A status is a claim about reality. The gate can contradict it.':'visitor operating guide',
 ...
 'A named, time-bounded exception. It permits a specific finding without deleting the rule.':'visitor operating guide',
 'A green result has to point at source, command, and tests. Green without that is not a delivery fact.':'visitor operating guide',
 "The gate runs inside the agent's loop. The agent gets the failure, fixes it, and reruns the gate before the work reaches review. CI should confirm that verdict, not discover it.":'visitor operating guide',
