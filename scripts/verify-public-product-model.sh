...
'docs/pack/guide.md':{
 ...
 'The minimum required to publish a pack today:':'author instruction',
 ...
 'Some Backstop-maintained packs already experiment with stronger publishing pipelines. These prototypes include:':'author instruction',
 '- running owner acceptance/fixture tests in CI\n- validating pack identity and version\n- checking that the Git tag matches `pack.yml`\n- preventing an existing version tag from being moved to another commit\n- creating the tag/release only after the publication checks pass':'author instruction',
 'Those pipelines are prototypes, not the required or canonical publishing model yet.':'author instruction',
 'Backstop is developing shared CI and publishing guidance for maintained packs. The current pipelines in some Backstop pack repositories are prototypes of that model; official guidance and reusable tooling are coming soon.':'author instruction'},
'docs/reference.md':{
 ...
 'Terminals: `replaced`, `canceled`, `deprecated`, `obsoleted`. Retired specs are exempt from live-work completeness.':'lifecycle machine framing',
 'Live: `idea` → `exploring` → `defined` → `ready`. Success terminal: `delivered`.':'lifecycle machine framing',
 'The user drives promotion. Do not self-promote.':'lifecycle machine framing',
 ...
 'Terminals without delivery: `replaced`, `canceled`, `deprecated`. `replaced` requires `replaced-by`.':'lifecycle machine framing',
 'Live: `draft` → `ready` → `implementing` → `completed`.':'lifecycle machine framing',
 '`draft`, `ready`, and `implementing` are the same shape to the validator: phases, tasks, tests before implementation, every source claim mapped. Terminal plans (`replaced`, `canceled`, `obsoleted`) are exempt from phase and task completeness.':'lifecycle machine framing',
 ...
 'Issue-sourced (`PLAN-ISSUE-NNN`): permits `delivered_by` on that issue. Spec-sourced (`PLAN-SPEC-NNN`): permits setting the spec to `implemented`. Bundle `delivered` is a separate declared status. Spec and bundle status are not inferred from the plan.':'lifecycle machine framing',
 '`replaced` requires `replaced-by`. `obsoleted` requires `obsoleted-by`.':'lifecycle machine framing',
 'Directive, ADR, and Capability have their own status vocabularies on [Directive](/directive/), [ADR](/adr/), and [Capability](/capability/). They are not paths into implementation.':'lifecycle machine framing'},
'docs/status.md':{
