...
'docs/pack/examples.md':{
 ...
 'Commit `backstop.yml` and `backstop.lock` with the change. Git packs later move with `backstop pack update`. Local packs refresh the lock with `backstop pack relock`. Command details live in Reference.':'consumer install guidance'},
'docs/pack/guide.md':{
