...
'docs/extend.md':{
 ...
 'If the standard is specific to this repository, keep the wiring here. Do not manufacture a pack that has no second consumer.':'author decision guidance',
 'If it should be a pack, author it next. [What is a pack?](/pack/) is the noun. [Choose a pack](/packs/#choose-a-pack) if a maintained pack already owns the standard.':'author decision guidance',
 ...
 '`--type` is `engine`, `mechanism`, or `toolchain`. The scaffold writes a valid `pack.yml` and a sample rule that can pass check, test, and the gate.':'author instruction',
 'A claim names what must be true. An engine checks it. Fixtures prove the check works, including negative cases that must fail.':'author instruction',
 'Rules explain the violation. Tool pins make execution reproducible. Iteration belongs inside the pack rather than in every consuming repository.':'author instruction',
 ...
 "Install the local pack into a consumer repository, then run that repository's gate. This proves the pack is consumed. It is not the Adopt walkthrough.":'author instruction',
...
'docs/reference.md':{
 ...
 'A spec comes from a bundle. Live specs carry requirements, claims, verification, implementation, and contracts. Entering `ready-for-implementation` is what makes planning legal. `implemented` means the work was delivered; the file stays a full spec.':'lifecycle machine framing',
