...
paths=['/','/evaluate/','/model/','/adopt/','/use-cases/','/pack/examples/','/pack/guide/','/reference/','/status/','/contributing/']
...
heroes=['What failure does Backstop prevent?','Your agent already writes the code.','How it works','Try it out.','Which problem-oriented adoption path applies?','Published packs.','Packs: a guide','What exact interface or behavior do I need?','What is supported, limited, planned, or intentionally outside Backstop?','How can I participate in Backstop and its ecosystem?']
...
neighborhood_owners=['/','/evaluate/','/evaluate/','/status/','/model/','/model/','/use-cases/','/pack/examples/','/pack/guide/','/reference/','/status/','/contributing/']
...
'CON-001':('Premise and trust thesis','decompose',['/evaluate/','/model/']),'CON-002':('Packs and zero bundled checks','decompose',['/model/','/pack/examples/']),'CON-003':('Thin-executor distinction','merge',['/model/']),'CON-004':('Gate, dimensions, severity, policy','decompose',['/model/','/reference/']),'CON-005':('Baselines and waivers','merge',['/model/']),'CON-006':('Artifacts and system composition','merge',['/model/']),
...
'PACK-001':('Pack definition and selection boundary','decompose',['/pack/','/pack/examples/']),'PACK-002':('Scaffold, manifest, and engine authoring','decompose',['/pack/guide/','/reference/']),'PACK-003':('Rules, claims, fixtures, and tools','decompose',['/pack/guide/','/reference/']),'PACK-004':('Path-filter sharp edge','decompose',['/reference/','/status/']),'PACK-005':('Check, test, findings, iteration','merge',['/pack/guide/']),'PACK-006':('Publishing and ecosystem continuation','decompose',['/pack/guide/','/contributing/']),
...
'PACK-004':[('/reference/','path-filter-diagnostics','slash-bearing include or exclude pattern'),('/status/','path-filter-limitation','Slash-bearing engine path patterns can fail open')],
...
'docs/model.md':{
 ...
 'Independent branches can execute in parallel. Dependencies establish the order when they cannot.':'visitor operating caption',
 ...
 '<div class="canonical-note" markdown="1">':'canonical source note',
 ...
 '<p class="figure-bridge">Packs compose across domains and toolchains.</p>':'visitor operating bridge',
 ...
 'Existing debt is not permission for new debt.':'visitor operating guide',
 ...
 'A claim names what must exist and the tests that prove it.':'visitor operating guide',
 ...
 '<div class="canonical-note" markdown="1">':'canonical source note',
 'The authoritative enforcement loop is `docs/_diagrams/ARCH-002-enforcement-loop.mmd`: intent bounds execution, pack engines return a verdict, and evidence feeds provenance back into intent.':'architecture-source identification',
 '<div class="canonical-note" markdown="1">\nThe authoritative enforcement loop is `docs/_diagrams/ARCH-002-enforcement-loop.mmd`: intent bounds execution, pack engines return a verdict, and evidence feeds provenance back into intent.':'architecture-source identification',
 ...
 "Backstop keeps the work inside the boundaries you define, so you can be confident you're getting the software you think you are.":'visitor operating closer',
...
'docs/pack/guide.md':{
 ...
 'An engine executes the rule. It is a runner. Semgrep, ast-grep, a shell script, and a native toolchain linter are examples, not the list. Packs are designed to support pretty much anything; that is why converters exist. The pack declares the engine. Many rules can share one engine.':'author instruction',
 ...
 'When the engine runs a native tool, the pack owns the conversion. `convert` is a pack-relative script. It reads the tool\'s stdout and emits SARIF. Core does not parse `grep`, `go test`, or `ast-grep` output.':'author instruction',
 'The script lives in the pack: `grep/to-sarif.sh`, `ast-grep/to-sarif.sh`, `scripts/build-to-sarif.sh`. Unparseable input must fail the convert, not be dropped. Most converters emit SARIF. A coverage engine converts to coverage records instead.':'author instruction',
 'If the tool already emits SARIF, omit `convert`.':'author instruction',
 'The sample sandbox validator has no convert: the validator is the logic.':'author instruction',
 '<div class="pack-model">\n<dl>\n<dt>Claim</dt>\n<dd>The definition. What must be true.</dd>\n<dt>Rule</dt>\n<dd>The implementation. How that requirement is checked.</dd>\n<dt>Engine</dt>\n<dd>The execution mechanism. The deterministic machinery that runs the rule.</dd>\n<dt>Fixtures</dt>\n<dd>Proof of the implementation. Known-positive and known-negative cases that demonstrate the rule behaves correctly.</dd>\n</dl>\n<p>The claim is the definition; the rule is its implementation; the engine executes that implementation. Fixtures exercise the rule against known cases to prove the implementation behaves correctly.</p>\n</div>':'author instruction',
 'A claim is the definition of what must be true. Declare it on the rule with an `id`, `text`, and both fixture polarities. There is no separate claims list.':'author instruction',
 'A rule implements the deterministic check for that claim. It names how a violation is reported. `risk_class` is `security`, `correctness`, `style`, or `perf`.':'author instruction',
 'The sample validator exits 0 on a clean file. It prints a message naming the target and exits non-zero when it fires. Replace the marker check `BACKSTOP-SAMPLE-VIOLATION` with the real detection.':'author instruction',
 'Fixtures are proof of the implementation. Every claim needs both polarities. Positive is known-good: the engine must stay silent. Negative is known-bad: the engine must fire.':'author instruction',
 'The sample positive file is clean. The sample negative file carries `BACKSTOP-SAMPLE-VIOLATION`. Rewrite both files when you replace the sample rule. Iteration belongs inside the pack rather than in every consuming repository.':'author instruction',
 'Set `input_scope` to `single-file` or `multi-file` when the rule should inspect consumer code. Without one, the sample rule only demonstrates pack check/test behavior.':'author instruction',
 '`pack check` validates the manifest. It does not run fixtures.':'author instruction',
 '<pre><code>backstop pack check ./my-standard</code></pre>':'author instruction',
 '`pack test` runs the engine against the declared fixtures and fails if the pair does not discriminate.':'author instruction',
 ...
 'Some Backstop-maintained packs use stronger CI publishing pipelines today, including owner acceptance, version/tag verification, and immutable release checks. These are prototypes, not the canonical publishing model. Shared CI guidance and reusable publishing tooling are coming soon.':'author instruction'},
'docs/pack/examples.md':{
 ...
 'Add the pack and version from the catalog:':'consumer install guidance',
 '<pre><code>backstop pack add backstop-ai/&lt;pack&gt;@&lt;version&gt;</code></pre>':'consumer install guidance',
 'That command clones the tagged release, validates it, installs it, merges configuration, pins the version in `backstop.yml`, and writes `backstop.lock`.':'consumer install guidance',
 'On a later clone, restore from the lock:':'consumer install guidance',
 ...
 "Run the project's gate:":'local pack install guidance',
 '<pre><code>backstop gate</code></pre>':'local pack install guidance',
 'After you edit the installed pack, refresh the lock:':'local pack install guidance',
 '<pre><code>backstop pack relock ./my-standard</code></pre>':'local pack install guidance',
 'Relock is local-source only. When the pack is ready to ship, [publish a tagged release](/pack/guide/#publish-a-pack) from its repository and add it with `backstop pack add org/pack@version`.':'local pack install guidance'},
'docs/reference.md':{
 ...
 'Terminals: `replaced`, `canceled`, `deprecated`, `obsoleted`. Retired specs are exempt from live-work completeness.':'terminal and successor rule',
 'Live: `idea` → `exploring` → `defined` → `ready`. Success terminal: `delivered`.':'per-noun live sequence',
 'The user drives promotion. Do not self-promote.':'lifecycle machine connective',
 ...
 'Terminals without delivery: `replaced`, `canceled`, `deprecated`. `replaced` requires `replaced-by`.':'terminal and successor rule',
 'Live: `draft` → `ready` → `implementing` → `completed`.':'per-noun live sequence',
 '`draft`, `ready`, and `implementing` are the same shape to the validator: phases, tasks, tests before implementation, every source claim mapped. Terminal plans (`replaced`, `canceled`, `obsoleted`) are exempt from phase and task completeness.':'plan same-shape rule',
 ...
 'Issue-sourced (`PLAN-ISSUE-NNN`): permits `delivered_by` on that issue. Spec-sourced (`PLAN-SPEC-NNN`): permits setting the spec to `implemented`. Bundle `delivered` is a separate declared status. Spec and bundle status are not inferred from the plan.':'terminal and successor rule',
 '`replaced` requires `replaced-by`. `obsoleted` requires `obsoleted-by`.':'terminal and successor rule',
 'Directive, ADR, and Capability each carry their own status vocabularies in their artifact schemas. They are not paths into implementation.':'vocabulary pointer'},
'docs/status.md':{
