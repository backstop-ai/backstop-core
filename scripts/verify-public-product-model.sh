...
'PACK-001':('Pack definition and selection boundary','decompose',['/packs/','/extend/']),'PACK-002':('Scaffold, manifest, and engine authoring','decompose',['/extend/','/reference/']),'PACK-003':('Rules, claims, fixtures, and tools','decompose',['/extend/','/reference/']),'PACK-004':('Path-filter sharp edge','decompose',['/reference/','/status/']),'PACK-005':('Check, test, findings, iteration','merge',['/extend/']),'PACK-006':('Publishing and ecosystem continuation','decompose',['/extend/','/contributing/']),
...
'docs/extend.md':{
 ...
 'It writes `pack.yml`, `validators/my-standard.sh`, `fixtures/valid/example.txt`, and `fixtures/invalid/example.txt`.':'author instruction',
 'An engine is the check. It is declared as data. The scaffold writes a sandbox engine whose validator executable is the logic, so the pack needs no external tool.':'author instruction',
 'Replace that sample with the real engine. `engine` means the validator is the logic. `mechanism` wraps a native tool. `toolchain` bundles a language\'s build, test, and lint passes.':'author instruction',
