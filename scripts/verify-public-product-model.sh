...
'docs/extend.md':{
 ...
 'A claim names the property that must stay true. Give it an `id` and `text`, and bind it to a rule. Every claim needs both fixture polarities.':'author instruction',
 'A rule binds an engine to those claims and names how a violation is reported. `risk_class` is `security`, `correctness`, `style`, or `perf`.':'author instruction',
 ...
 'The sample rule declares no `input_scope`, so at gate time it does not scan consumer files. Set `input_scope` to `single-file` or `multi-file` when the rule should inspect the project.':'author instruction',
