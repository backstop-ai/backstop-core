...
'docs/pack/guide.md':{
 ...
 'A claim names the property that must stay true. Declare it on the rule with an `id`, `text`, and both fixture polarities. The rule owns the claim; there is no separate claims list.':'author instruction',
 'A rule binds an engine to its claims and names how a violation is reported. `risk_class` is `security`, `correctness`, `style`, or `perf`.':'author instruction',
 ...
 'Every claim needs both polarities. Positive is known-good: the engine must stay silent. Negative is known-bad: the engine must fire.':'author instruction',
