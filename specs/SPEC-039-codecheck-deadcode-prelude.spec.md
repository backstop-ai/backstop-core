...
- **1.1.0** (2026-07-03) — Status → `implemented`. BUNDLE-011 Seed 1 dead-code prelude
  ...
  claim, contract, or prose change.
- **1.0.0** (2026-06-24) — Initial spec: the BUNDLE-011 Seed 1 behavior-preserving dead-code
  prelude — delete the vestigial `.manifest.json` reader (compiled + legacy arms) and its
  stranded rule-matching path (REQ-010), and the already-no-op non-Go findings catch-all in
  `routeFileDefaults` (REQ-007), shrinking the surface for the Seed 2 / SPEC-040 cutover.
