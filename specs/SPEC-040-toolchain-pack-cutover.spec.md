...
contracts:
  ...
  - file: cmd/backstop/pack_gate.go
    provides:
      ...
      - name: dispatchBuildViolationProjectWide
        kind: variable
        signature: "gate.Violation.ProjectWide (set true for build-pass engine violations)"
        notes: >
          The TRANSITIONAL build-exemption SEAM (Sharp Edge 2). Today the build-pass
          scope-filter exemption is set in checkViolationsToGate (`cv.Pass ==
          check.CheckTypeBuild` → gate.Violation.ProjectWide=true, gate.go:1173) and consumed by
          pkg/gate/scope.go:194 to skip diff-scope filtering for build breaks. This cutover
          DELETES realCodeChecker/checkViolationsToGate, so engine-path build violations would
          carry ProjectWide=false and a build break in an UNCHANGED file would be silently
          scope-filtered away — an under-broad vacuous-green regression in the SPEC-040→SPEC-041
          window. Transitional contract (REQ-001/CLM-029): the engine dispatch path must set
          ProjectWide=true for build-pass violations (transitional preservation) — OR SPEC-040
          and SPEC-041 land in LOCKSTEP. SPEC-041 REQ-004 replaces this transitional seam with a
          declared `exempt_from_scope_filter` engine property mapping to Violation.ProjectWide;
          this contract is the bridge until that lands. NOT a new permanent mechanism — a
          guarded transitional preservation, mirroring the coverage seam (CLM-028).
    consumes:
