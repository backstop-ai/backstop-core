...
func TestCLI_ExitCode_OnlyValidCodes(t *testing.T) {
	cases := []struct {
		...
		{"pass no error", ValidationResult{Pass: true}, nil},
		{"violations no error", ValidationResult{Pass: false, Violations: []string{"v1"}}, nil},
		{"pass with error", ValidationResult{Pass: true}, errForTest("err")},
		{"violations with error", ValidationResult{Pass: false, Violations: []string{"v1"}}, errForTest("err")},
		{"empty result no error", ValidationResult{}, nil},
		{"empty result with error", ValidationResult{}, errForTest("err")},
	}
