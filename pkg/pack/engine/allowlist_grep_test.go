...
// allowlist_grep_test.go (SPEC-038 TASK-005, REQ-005): the grep allowlist gating
// matrix. The grep engine is pack-declared (no DefaultRegistry entry), so the ONLY
// thing that lets a pack-declared grep command clear the trust floor is grep being
// on the backstop-owned TrustedToolAllowlist at a pinned version (CLM-016). The
// allowlist gating is the tested invariant: allowlisted+pinned → runs (CLM-017);
// removed from the allowlist → loud un-allowlisted ConfigError (CLM-018). This
// lands ATOMICALLY with the grep engine declaration (Sharp Edge 6).

// TestAllowlist_GrepPresentPinned asserts grep is present on the backstop-owned
// trusted-tool allowlist at a pinned (non-empty) version — the trust floor a
// pack-declared grep command must clear (CLM-016).
func TestAllowlist_GrepPresentPinned(t *testing.T) {
	al := TrustedToolAllowlist()
	for _, tool := range []string{"grep"} {
		pinned, ok := al[tool]
		if !ok {
...
// TestAllowlist_GrepUnallowlistedFailsLoud asserts that when grep is removed from
// the allowlist, a pack-declared grep command is NOT run — CheckToolAllowed
...
func TestAllowlist_GrepUnallowlistedFailsLoud(t *testing.T) {
	// Build an allowlist WITHOUT grep to model the removed-from-allowlist state.
