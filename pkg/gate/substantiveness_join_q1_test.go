...
// maps each routed hollow finding back to its mandated test (by File + FuncName
// parsed from the PINNED `func=<FN>` message) and reports whether a given test is
// hollow — language-agnostically (Go and TS findings flow identically). Driven by
// SYNTHETIC routed findings here; the REAL ast-grep end-to-end assertions (the
// mandated CLM-003/004/012/013/014 names) live in the Phase-3 real-engine file.
...
func TestQ1Helper_RoutedHollow_PerTestVerdict(t *testing.T) {
	hollow := []Violation{
		{Rule: testHollowRuleID, File: "x_test.go", Message: "test function TestX has no assertions (hollow) func=TestX"},
	}
...
// one whose func= matches the hollow finding is hollow, proving the verdict keys on
// (File, FuncName), not file alone.
func TestQ1Helper_HollowKeyedByFuncNotJustFile(t *testing.T) {
	hollow := []Violation{
		{Rule: testHollowRuleID, File: "shared_test.go", Message: "test function TestHollow has no assertions (hollow) func=TestHollow"},
	}
	...
	if !IsTestHollow(hollow, hollowTest) {
		t.Errorf("TestHollow should be hollow (its func= matches)")
	}
...
// TestQ1Helper_TsHollowFlowsIdentically — a TS hollow finding (parallel `func=`
// shape) keys the same way as a Go finding, proving the helper is language-agnostic
// (the gate consumption the TS proof rides — CLM-014's gate side).
func TestQ1Helper_TsHollowFlowsIdentically(t *testing.T) {
	hollow := []Violation{
		{Rule: "backstop/ts-proof/hollow-test-ts", File: "hollow.test.ts", Message: "hollow test func=ts-hollow"},
	}
	tsTest := MandatedTest{FuncName: "ts-hollow", FilePath: "hollow.test.ts"}
	if !IsTestHollow(hollow, tsTest) {
		t.Errorf("a TS hollow finding with a parallel func= shape must flow through IsTestHollow identically")
	}
}
