...
// rule path's BASE NAME is the captured rule file, so the fixture reaches the recipe
// directory by copy rather than by a re-typed rule body.
...
//
// The engine not being runnable is a FAILURE, never a skip: a skipped transform test
// is indistinguishable from a passing no-op.
...
// vacuous (a neutral extension the engine's parser ignores, say) fails here loudly
// instead of letting a no-op dispatch pass.
func TestApply_TransformOp_DispatchesToAllowlistedEngine_CapturedFixture(t *testing.T) {
	recipeDir := t.TempDir()
	projectRoot := t.TempDir()

	resolved := resolvedFromManifest(t, recipeDir, capturedTransformRecipe)
	...
	copyUnder(t, recipeDir, transformOp.Rule, capturedTransformPath(filepath.Base(transformOp.Rule)))
...
// recipe's DECLARED paths resolved under the recipe directory and the project root,
// and swapping the declared rule for a second one swaps what the dispatch receives.
//
...
transform_rules:
  - rules/rewrite-rule.yml
  - rules/second-rule.yml
ops:
  - id: op-captured-transform
    kind: transform
    target: source/greeting.go
    rule: rules/second-rule.yml
    manual: "Re-point the superseded call sites in the declared target by hand."
`
	...
	applyWithRecorder := func(t *testing.T, recipeYAML string) (*ResolvedRecipe, string, string, recorded) {
		...
		transformOp := resolved.Manifest.Ops[0]
		copyUnder(t, recipeDir, transformOp.Rule, capturedTransformPath(capturedTransformRuleName))
		...
		return resolved, recipeDir, projectRoot, seen
	}

	firstResolved, firstRecipeDir, firstProjectRoot, first := applyWithRecorder(t, capturedTransformRecipe)
	...
	if want := filepath.Join(firstRecipeDir, firstOp.Rule); first.rule != want {
		t.Errorf("dispatched rule = %q, want the DECLARED rule resolved under the recipe directory %q", first.rule, want)
	}
	...
	secondResolved, secondRecipeDir, _, second := applyWithRecorder(t, secondRuleRecipe)
	...
	if want := filepath.Join(secondRecipeDir, secondOp.Rule); second.rule != want {
